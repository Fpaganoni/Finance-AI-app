...
export function Hero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (sectionRef.current) {
        ...
        // background can play out a full transition instead of snapping
  ...
  return (
    <section ref={sectionRef} className="relative h-[260vh]">
      <div
        ref={pinRef}
        className="sticky top-0 h-screen w-full flex flex-col items-center justify-center px-6 pt-24 pb-16 overflow-hidden"
      >
        {/* Ambient gradient-mesh blobs, drifting behind the 3D canvas */}
        <div className="pointer-events-none absolute inset-0 -z-20 overflow-hidden">
          <div className="hero-blob hero-blob-a absolute w-152 h-152 rounded-full bg-accent/25 blur-[110px]" />
          <div className="hero-blob hero-blob-b absolute w-120 h-120 rounded-full bg-accent/15 blur-[100px]" />
          <div className="hero-blob hero-blob-c absolute w-[24rem] h-96 rounded-full bg-primary/10 blur-[90px]" />
        </div>
